...
export default function ArtSection({ id, title, images, variant = "gallery" }: ArtSectionProps) {
  ...
  return (
    <section id={id} className="w-full">
      {/* SECTION HEADER - FIXED: Removed font-bold */}
      <div className="flex items-end justify-between mb-8 border-b border-white/10 pb-4">
        <h3 className="text-3xl md:text-4xl font-silkscreen text-white tracking-tight">
          {title}
        </h3>
        <span className="font-mono text-xs text-[#4ADE80]">[{images.length} ASSETS]</span>
      </div>
            ...
            <div
              ...
              className={`
                ...
                ${isComic
                  ...
                  : "bg-slate-900 border border-white/10 hover:border-[#4ADE80] transition-colors duration-300 mb-4"
                }
              `}
            >
              <img
              ...
              {isComic && (
                <div className="p-4 bg-[#0a0a0a] border-t border-slate-800">
                  <p className="text-[#4ADE80] font-silkscreen text-xs mb-1 tracking-widest">ISSUE #{i + 1}</p>
              ...
              {!isComic && (
                <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-transparent to-transparent opacity-0 hover:opacity-100 transition-opacity duration-300 flex items-end p-4">
                   {description ? (
                      <p className="text-sm font-medium text-white leading-tight font-mono">
                        {description}
                      </p>
                   ) : (
                      <p className="text-xs font-silkscreen text-[#4ADE80]/50">REXTOON</p>
                   )}
                </div>
              )}
            </div>
